...
def delete_item(fbx_list :any) -> None:
    ...
    target_node_path = f"/obj/shelf_dop/{file_name}"
    ...
    assign_id_wrangle_node = hou.node("/obj/shelf_dop/attribwrangle_assign_id")
    if assign_id_wrangle_node:
        all_weights = create_weight_str()
        set_weight_to_wrangle(assign_id_wrangle_node, all_weights)


def update_node(file_path: str, scale_param: float, rotation: tuple[float, float, float], maxstack_num: int, stack_seed: int, weight: int) -> None:
    file_name: str = pathlib.Path(file_path).name
    settings = load_setting()
    if file_path not in settings:
        print(f"{file_path}は追加されていません")
        return
    save_json(file_path, scale_param, rotation, maxstack_num, stack_seed, weight)
    all_weights = create_weight_str()
    file_node = hou.node(f"/obj/shelf_dop/{file_name}/file1")
    trans_node = hou.node(f"/obj/shelf_dop/{file_name}/transform1")
    wrangle_node = hou.node(f"/obj/shelf_dop/{file_name}/attribwrangle1")
    assign_id_wrangle_node = hou.node("/obj/shelf_dop/attribwrangle_assign_id")
    ...
    wrangle_node.parm("snippet").set(
        f"""int seed = {stack_seed};
        int maxstack_num = {maxstack_num} + 1;
        i@stack_num = int(fit(rand(seed),0,1,1,maxstack_num
        ));""")
    
    set_weight_to_wrangle(assign_id_wrangle_node, all_weights)


def create_file_node(fbx_list: any, file_path: str, scale_param: float, rotation: tuple[float, float, float], maxstack_num: int, stack_seed: int, weight: int) -> None:
    file_name: str = pathlib.Path(file_path).name
    settings = load_setting()
    ...
    shelf_geometry_node = hou.node("/obj/shelf_dop")
    ...
    merge_node = hou.node("/obj/shelf_dop/merge_fbx")
...
//重み間での自動生成
float r = rand(@class + chi("seed")) * total_w;
float num = 0;
i@fbx_id = 0;
// 累積判定のループ
...
def save_json(file_path: pathlib.Path, scale_param: float, rotation: tuple[float, float, float], maxstack_num: int, stack_seed: int, weight: int) -> None:
    settings = load_setting()
    if file_path not in settings:
        settings[file_path] = {}
    ...
    settings[file_path]["stack_seed"] = stack_seed
...
def on_selected_Item(fbx_list: QtWidgets.QListWidget, path_line: QtWidgets.QLineEdit, 
                     ...
                     seed_spinbox: QtWidgets.QDoubleSpinBox,
    ...
    item_stackseed = current_item_setting.get('stack_seed', 1)
    ...
    seed_spinbox.setValue(item_stackseed)
...
class mainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.seed_spinbox = settings_spinbox(1.0, 100, 0, 1)
    ...
    def set_import_layout(self) -> QtWidgets.QVBoxLayout:
        ...
        param_layout.addWidget(QtWidgets.QLabel("Stack_Seed"), 3, 0)
        param_layout.addWidget(self.seed_spinbox, 3, 1)
    ...
    def set_list_layout(self) -> QtWidgets.QVBoxLayout:
        ...
        self.fbx_list.itemClicked.connect(lambda: on_selected_Item(
            ...
            self.stacknum_spinbox, self.seed_spinbox, self.weight_spinbox))
